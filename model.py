...
def modelo(df_notas):
    ...
    model += lpSum([cant_unidades[id_nota, id_producto] - df_notas.loc[(id_nota, id_producto), 'inventario'].sum() for id_nota, id_producto in df_notas.index]) <= 0

    # (2) La cantidad tiene que ser igual o menor a lo que solicitó el cliente:
    model += lpSum([cant_unidades[id_nota, id_producto] - df_notas.loc[(id_nota, id_producto), 'cantidad'].sum() for id_nota, id_producto in df_notas.index]) <= 0
